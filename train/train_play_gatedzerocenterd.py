...
# %%
def save_checkpoint(model: Gemma3, cfg: Config, optimizer: CustomOptimizer, step: int, checkpoint_dir: str = "checkpoints", is_best: bool = False):
...
save_cfg_dict["RADAM_SF_LR"] = RADAM_SF_LR
save_cfg_dict["BETA1"] = BETA1
save_cfg_dict["WEIGHT_DECAY"] = WEIGHT_DECAY
run = wandb.init(project="gemma3_play", config=save_cfg_dict, name=TIME_STR+f"_gatedzerocenterd_radam-lr{RADAM_SF_LR}_muon-lr{MUON_LR}_beta1_{BETA1}_weight_decay_{WEIGHT_DECAY}", sync_tensorboard=True)
